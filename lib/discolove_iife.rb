class DiscoloveIIFE
  ...
  def call(input)
    ...
    # Only discolove or admin paths
    return data unless (path =~ /\/javascripts\/discolove/ || path =~ /\/javascripts\/admin/ || path =~ /\/test\/javascripts/)
    ...
    return data if path =~ /discolove-loader/
