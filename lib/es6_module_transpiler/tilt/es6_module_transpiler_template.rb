...
JS
    ...
    def whitelisted?(path)
      ...
      @@whitelisted ||= Set.new(
        ["discolove/models/nav-item",
         "discolove/models/user-action",
         "discolove/routes/discolove",
         "discolove/models/category",
         "discolove/models/trust-level",
         "discolove/models/site",
         "discolove/models/user",
         "discolove/models/session",
         "discolove/models/model",
         "discolove/views/grouped"]
      )

      @@whitelisted.include?(path) || path =~ /discolove\/mixins/
    end
    ...
    def evaluate(scope, locals, &block)
      ...
      if whitelisted?(scope.logical_path) &&
        scope.logical_path =~ /(discolove|admin)\/(controllers|components|views|routes|mixins|models)\/(.*)/
        ...
        if require_name !~ /\-test$/ && require_name !~ /^discolove\/plugins\//
          result = "#{class_name}#{type.classify}"

          # HAX
          result = "Controller" if result == "ControllerController"
          result = "Route" if result == "DiscoloveRoute"
          ...
          if result != "PostMenuView"
            @output << "\n\nDiscolove.#{result} = require('#{require_name}').default;\n"
          end
        end
      end
    ...
    def module_name(root_path, logical_path)
      ...
      if root_path =~ /(.*\/#{root_base}\/plugins\/[^\/]+)\//
        ...
        plugin = Discolove.plugins.find {|p| p.path == plugin_path }
        path = "discolove/plugins/#{plugin.name}/#{logical_path.sub(/javascripts\//, '')}" if plugin
      end
